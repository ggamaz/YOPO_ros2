...
class OutputData(metaclass=Metaclass_OutputData):
    ...
    def __init__(self, **kwargs):
        ...
        if 'radio_channel' not in kwargs:
            ...
            self.radio_channel = numpy.array(kwargs.get('radio_channel'), dtype=numpy.uint8)
            assert self.radio_channel.shape == (8, )
    ...
    def __eq__(self, other):
        ...
        if self.magnetic_field != other.magnetic_field:
            return False
        if all(self.radio_channel != other.radio_channel):
            return False
