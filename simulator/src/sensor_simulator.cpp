...
cv::Mat SensorSimulator::renderDepthImage() {
  cv::Mat depth_image(
    image_height, image_width, CV_32FC1, cv::Scalar(std::numeric_limits<float>::max()));
    ...
    for (int h = 0; h < horizontal_num; ++h) {
      ...
      if (octree->getIntersectedVoxelIndices(ray_origin, ray_direction, pointIdxVec, 1)) {
        ...
        if (max_lidar_dist > point_in_body.norm())
          line_clouds[v].points.push_back(
            pcl::PointXYZ(point_in_body.x(), point_in_body.y(), point_in_body.z()));
      }
    }
  }
  auto end = std::chrono::high_resolution_clock::now();
...
void SensorSimulator::expand_cloud(pcl::PointCloud<pcl::PointXYZ>::Ptr expanded_cloud,
                                   int direction) {
  auto start = std::chrono::high_resolution_clock::now();
