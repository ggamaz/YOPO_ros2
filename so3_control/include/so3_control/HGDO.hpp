...
public:
  HGDO(){};
  HGDO(double control_dt) { control_dt_ = control_dt; };
  ...
  void HGDO_ext_force_ob(const Eigen::Vector3d &U_input,
                         const Eigen::Vector3d &vel, Eigen::Vector3d &dis) {
  ...
  // 自适应干扰观测，立文讲先不用
  void AHGDO_ext_force_ob(Eigen::Vector3d &U_input, Eigen::Vector3d &vel,
                          Eigen::Vector3d &dis) {
    Eigen::Vector3d dz1, dz2;
    Eigen::Vector3d U = U_input;
    // U(2) += g;
    for (int i = 0; i < 3; i++) {
      ...
      dz1(i) = U(i) + z2_nhgdo(i) -
               (alpha_1 / theta_1) * (z1_nhgdo(i) - vel(i)) -
               (alpha_1 * d_bound * (1 / theta_2 - 1 / theta_1)) * Sat(ob);
      dz2(i) =
          -(alpha_2 / pow(theta_1, 2)) * (z1_nhgdo(i) - vel(i)) -
          (alpha_2 * d_bound * (1 / pow(theta_2, 2) - 1 / pow(theta_2, 2))) *
              Sat(ob);
    }
    for (int i = 0; i < 3; i++) {
