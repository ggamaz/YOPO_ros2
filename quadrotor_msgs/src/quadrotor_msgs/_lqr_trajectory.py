...
class LQRTrajectory(metaclass=Metaclass_LQRTrajectory):
    ...
    def __init__(self, **kwargs):
        ...
        if 's0' not in kwargs:
            ...
            self.s0 = numpy.array(kwargs.get('s0'), dtype=numpy.float64)
            assert self.s0.shape == (6, )
        if 'ut' not in kwargs:
            self.ut = numpy.zeros(3, dtype=numpy.float64)
        else:
            self.ut = numpy.array(kwargs.get('ut'), dtype=numpy.float64)
            assert self.ut.shape == (3, )
        if 'sf' not in kwargs:
            self.sf = numpy.zeros(6, dtype=numpy.float64)
        else:
            self.sf = numpy.array(kwargs.get('sf'), dtype=numpy.float64)
            assert self.sf.shape == (6, )
    ...
    def __eq__(self, other):
        ...
        if self.final_yaw != other.final_yaw:
            return False
        if all(self.s0 != other.s0):
            return False
        if all(self.ut != other.ut):
            return False
        if all(self.sf != other.sf):
            return False
