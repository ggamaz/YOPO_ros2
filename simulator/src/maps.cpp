  ...
  *info.cloud += *ground_cloud;
  ...
  if (_wall_ceiling) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr ceiling_cloud =
      generateGround(info.cloud, _resolution, _h_h);
    ...
    for (int j = 0; j < info.sizeY; ++j) {
      for (int k = 0; k < info.sizeZ; ++k) {
        ...
        for (int it = 1; it <= fractal; ++it) {
          ...
          tnoise +=
            ta * noise.noise(dfv * i * complexity, dfv * j * complexity, dfv * k * complexity);
        }
        v->push_back(tnoise);
      }
    }
...
void Maps::recursiveDivision(int xl, int xh, int yl, int yh, Eigen::MatrixXi &maze) {
  ...
  if (xl < xh - 3 &&
      yl < yh - 3) {  // the remaining area is larger than or equal to 5*5, need to add both x
    // wall and y wall
...
void Maps::recursizeDivisionMaze(Eigen::MatrixXi &maze) {
  ...
  if (px != 1 && px != (sx - 2)) {
    for (int i = 1; i < (sy - 1); ++i) {
      if (i != y1 && i != y2) maze(px, i) = 1;
    }
  }
  if (py != 1 && py != (sy - 2)) {
    for (int i = 1; i < (sx - 1); ++i) {
      if (i != x1 && i != x2) maze(i, py) = 1;
    }
  }
  switch (std::rand() % 4) {
    case 0:
...
Maps::BasicInfo Maps::getInfo() const { return info; }

void Maps::setInfo(const BasicInfo &value) { info = value; }

void Maps::setParam(const YAML::Node &config) {
  // perlin3D
...
pcl::PointXYZ MazePoint::getPoint() { return point; }

int MazePoint::getPoint1() { return point1; }

int MazePoint::getPoint2() { return point2; }

double MazePoint::getDist1() { return dist1; }

double MazePoint::getDist2() { return dist2; }

void MazePoint::setPoint(pcl::PointXYZ p) { point = p; }

void MazePoint::setPoint1(int p) { point1 = p; }

void MazePoint::setPoint2(int p) { point2 = p; }

void MazePoint::setDist1(double set) { dist1 = set; }

void MazePoint::setDist2(double set) { dist2 = set; }
...
void Maps::Maze3DGen() {
  ...
  for (int i = 0; i < numNodes; i++) {
    double rx = std::rand() / RAND_MAX + (std::rand() % info.sizeX) / info.scale -
                info.sizeX / (2 * info.scale);
    double ry = std::rand() / RAND_MAX + (std::rand() % info.sizeY) / info.scale -
                info.sizeY / (2 * info.scale);
    double rz = std::rand() / RAND_MAX + (std::rand() % info.sizeZ) / info.scale -
  ...
  for (int i = 0; i < info.sizeX; i++) {
    for (int j = 0; j < info.sizeY; j++) {
      for (int k = 0; k < info.sizeZ; k++) {  // for every scaled coordinate points
        ...
        test.z = k / info.scale -
                 info.sizeZ / (2 * info.scale);  // marking the corresponding point location
        ...
        for (int ii = 0; ii < numNodes; ii++) {
          double dist = std::sqrt((base[ii].x - test.x) * (base[ii].x - test.x) +
        ...
        if (std::abs(mp.getDist2() - mp.getDist1()) <
          ...
          if ((mp.getPoint1() + mp.getPoint2()) > int((1 - connectivity) * numNodes) &&
            ...
            double judge = std::sqrt((base[mp.getPoint1()].x - base[mp.getPoint2()].x) *
...
void Maps::forest() {
  ...
  if (pcl::io::loadPLYFile(tree_file, *tree_cloud) == -1) {
    std::cerr << "Error loading tree model from file: " << tree_file << std::endl;
    return;
  }
  ...
  std::cout << "Generated " << positions.size() << " positions for trees." << std::endl;
  // 生成森林点云
  ...
  std::cout << "Generating forest with " << positions.size() << " trees." << std::endl;
  ...
  std::cout << "Generated " << info.cloud->points.size() << " points in the forest." << std::endl;
  // 生成地面点云
  pcl::PointCloud<pcl::PointXYZ>::Ptr ground_cloud = generateGround(info.cloud, _resolution);
  *info.cloud += *ground_cloud;
...
void Maps::generatePoissonPoints(float map_width,
                                 float map_height,
                                 float dist,
                                 std::vector<Eigen::Vector2f> &positions) {
...
void Maps::scaleAndTranslateCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                                  float scale_factor,
...
pcl::PointCloud<pcl::PointXYZ>::Ptr Maps::generateGround(
  const pcl::PointCloud<pcl::PointXYZ>::Ptr &forest_cloud, float grid_size, float height) {
  ...
  for (float x = x_min; x <= x_max; x += grid_size) {
    for (float y = y_min; y <= y_max; y += grid_size) {
      ground_cloud->emplace_back(x, y, height);
    }
  }
  return ground_cloud;
}
...
void Maps::room() {
  ...
  Eigen::Matrix3f rotation90 =
    Eigen::AngleAxisf(M_PI / 2, Eigen::Vector3f::UnitZ()).matrix();  // 90度旋转
  ...
  if (add_ceiling) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr ground_cloud = generateGround(info.cloud, _resolution);
    pcl::PointCloud<pcl::PointXYZ>::Ptr ceiling_cloud =
      generateGround(info.cloud, _resolution, room_H - _resolution);
    *info.cloud += *ground_cloud;
    *info.cloud += *ceiling_cloud;
  }
  info.cloud->width = info.cloud->points.size();
...
// 生成带窗户的基础墙体
void Maps::generateWallWithWindows(
  pcl::PointCloud<pcl::PointXYZ>::Ptr wall, float L, float W, float H, int num_windows) {
  // 存储每个窗户的位置和大小
...
void Maps::transformPointCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud,
