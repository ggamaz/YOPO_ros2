...
#include "quadrotor_simulator/Quadrotor.hpp"
#include "uav_utils/geometry_utils.hpp"
...
void odomToMesh(const nav_msgs::msg::Odometry &odom_msg, visualization_msgs::msg::Marker &meshROS);
...
void cmd_callback(const quadrotor_msgs::msg::SO3Command::ConstSharedPtr &cmd) {
...
void force_disturbance_callback(const geometry_msgs::msg::Vector3::ConstSharedPtr &f) {
  disturbance.f(0) = f->x;
  disturbance.f(1) = f->y;
  disturbance.f(2) = f->z;
}

void moment_disturbance_callback(const geometry_msgs::msg::Vector3::ConstSharedPtr &m) {
  disturbance.m(0) = m->x;
  disturbance.m(1) = m->y;
  disturbance.m(2) = m->z;
}
...
int main(int argc, char **argv) {
  ...
  auto n = std::make_shared<rclcpp::Node>("quadrotor_simulator_so3", options);
  ...
  auto cmd_sub = n->create_subscription<quadrotor_msgs::msg::SO3Command>("cmd", 100, &cmd_callback);
  ...
  _init_x = n->declare_parameter("simulator/init_state_x", 0.0);
  _init_y = n->declare_parameter("simulator/init_state_y", 0.0);
  _init_z = n->declare_parameter("simulator/init_state_z", 1.0);

  Eigen::Vector3d position = Eigen::Vector3d(_init_x, _init_y, _init_z);
  quad.setStatePos(position);
  RCLCPP_INFO(n->get_logger(), "Quadrotor Initial Position: %f, %f, %f", _init_x, _init_y, _init_z);

  double simulation_rate = n->declare_parameter("rate/simulation", 1000.0);
  if (simulation_rate <= 0) {
    RCLCPP_FATAL(n->get_logger(), "Simulation rate must be positive.");
    return -1;
  }

  double odom_rate = n->declare_parameter("rate/odom", 100.0);
  if (odom_rate <= 0) {
    RCLCPP_FATAL(n->get_logger(), "Odometry rate must be positive.");
    return -1;
  }
  const rclcpp::Duration odom_pub_duration(1.0 / odom_rate, 0);

  std::string quad_name = n->declare_parameter("quadrotor_name", std::string("quadrotor"));
...
void odomToMesh(const nav_msgs::msg::Odometry &odom_msg, visualization_msgs::msg::Marker &meshROS) {
  ...
  //                         ros::package::getPath("so3_quadrotor_simulator") +
  //                         "/config/uav.dae";
  std::string package_share_directory =
      ament_index_cpp::get_package_share_directory("so3_quadrotor_simulator");
  meshROS.mesh_resource = package_share_directory + "/config/uav.dae";
